...
def plan_file_creation(initial_message: str, file_name: str):
    statement = f"<user_prompt>{initial_message}</user_prompt>: to start with we will narrow our focus on {file_name} "
...
def write_file(file_name: str, purpose: str):
    statement = f"Write/Rewrite {file_name} based on your previous plan of action for this particular file, "
...
def write_code_file(file_name: str, purpose: str):
    statement = f"Write/Rewrite {file_name} based on your previous plan of action and the actual contents of "
